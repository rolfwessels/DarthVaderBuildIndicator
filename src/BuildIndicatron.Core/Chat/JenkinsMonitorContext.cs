    ...
    public class JenkinsMonitorContext : TextSplitterContextBase<JenkinsMonitorContext.Meta>, IWithHelpText
    {
            ...
            Meta server)
        {
            ...
            if (!string.IsNullOrEmpty(server.Where))
                await context.Respond(string.Format("Im currently monitoring jenkins on {0} for projects {1}",
                    buildChannel, buildChannels.StringJoin()));
        ...
        public IEnumerable<HelpMessage> GetHelp()
        {
            ...
            {
                Call = "where are you monitoring jenkins",
                Description = "See where jenkins is being monitored."
            };
