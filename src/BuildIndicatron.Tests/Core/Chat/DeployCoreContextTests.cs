...
namespace BuildIndicatron.Tests.Core.Chat
{
    ...
    public class DeployCoreContextTests : ChatBotTestsBase
    {
        ...
        [Test]
        public async Task Process_GivenSetSettingsWithSpace_ShouldResondWithDeployCoreContext()
        {
            // arrange
            Setup();
        ...
        [Test]
        public async Task Process_GivenSetSettingsWithSpaceBrackets_ShouldResondWithDeployCoreContext()
        {
            // arrange
            Setup();
        ...
        public async Task Process_GivenOldSetup_ShouldNotTransferValue()
        {
            ...
            await _chatBot.Process(messageContext = new MessageContext("cancel"));
            messageContext.LastMessages.Last().Should().Be("nevermind");
            await _chatBot.Process(messageContext = new MessageContext("set setting"));
            messageContext.LastMessages.Last().Should().Be("what is the key?");
            // assert
        }
