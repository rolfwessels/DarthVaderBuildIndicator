...
namespace BuildIndicatron.Tests.Processes
{
    ...
    public class GoogleTextToSpeachTests
    {
        ...
        public void Split_GivenALongerString_ShouldCallDownload()
        {
            ...
            // action
            var split = _googleTextToSpeach.Split(
                "This is some very long text , that just goes on and on. and on. This is some very long text , that just goes on and on. and on.");
            // assert
        ...
        public void Play_GivenFullImplementation_ShouldPlayFile()
        {
            Setup();
            // arrange
            _googleTextToSpeach = new GoogleTextToSpeach(new DownloadToFile("./", new SettingsManager("settings.json")),
                new Mp3Player());
            // action
